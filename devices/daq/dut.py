...
# application developer. Thus outside of daq. Otherwise multiple calls to
...
class Dut(Base):
    ...
    def load_hw_configuration(self, conf, extend_config=False):
        ...
        if 'transfer_layer' in conf:
            for intf in conf['transfer_layer']:
                ...
                kargs = {}
                kargs['conf'] = intf
                self._transfer_layer[intf['name']] = self._factory('daq.TL.' + intf['type'], *(), **kargs)
            ...
            if conf['hw_drivers']:
                for hwdrv in conf['hw_drivers']:
                    ...
                    if 'interface' in hwdrv:
                        ...
                        kargs['intf'] = None
                    kargs['conf'] = hwdrv
                    self._hardware_layer[hwdrv['name']] = self._factory('daq.HL.' + hwdrv['type'], *(), **kargs)
        ...
        if 'user_drivers' in conf:
            ...
            if conf['user_drivers']:
                for userdrv in conf['user_drivers']:
                    ...
                    kargs['conf'] = userdrv
                    self._hardware_layer[userdrv['name']] = self._factory('daq.HL.' + userdrv['type'], *(), **kargs)
            ...
            if conf['registers']:
                for reg in conf['registers']:
                    ...
                    if 'driver' in reg:
                        ...
                        kargs['conf'] = reg
                        self._registers[reg['name']] = self._factory('daq.RL.' + reg['type'], *(), **kargs)
    ...
    def _factory(self, importname, *args, **kargs):
        ...
        try:
            ...
            if len(splitted_import_name) > 2 and splitted_import_name[0] == 'daq':
                importname = '.'.join(splitted_import_name[2:])  # remove "daq.RL." etc.
                mod = import_module(importname)
            else:  # raise initial exception
                raise
        daq_base_classes = getmembers(mod, is_daq_base_class)
        cls = None
        if not daq_base_classes:  # found no base class
            raise ValueError('Found no matching class in %s.' % importname)
        elif len(daq_base_classes) > 1:  # found more than 1 base class
            mod_name = splitted_import_name[-1]
            for daq_base_class in daq_base_classes:
                if mod_name == daq_base_class[0]:  # check for base class name
                    cls = daq_base_class[1]
                    break
            if cls is None:
                raise ValueError('Found more than one matching class in %s.' % importname)
        else:  # found single class
            cls = daq_base_classes[0][1]
