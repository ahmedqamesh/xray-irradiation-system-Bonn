...
#   p.calibration_temperature(data="temperature_dose.h5", Directory=Directory, PdfPages=PdfPages)
    ...
    p.dose_current(stdev=0.04, PdfPages=PdfPages, directory=test_directory, test=tests[0], depth= depth, table=False, voltages=voltages)
    #p.dose_drop(stdev=0.04, PdfPages=PdfPages, tests = tests, directory=test_directory,depth= depth,voltages=["40kV", "30kV"])
    p.dose_voltage(PdfPages=PdfPages, directory=test_directory ,test=tests[0], kafe_Fit=True, table=False, currents = currents)
